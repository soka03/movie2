...
def main_process(file_path):
    ...
    # 요청 보낼 데이터 생성(sample_size 설정 가능, sample_size = 'max'로 설정 시 전체 데이터 모두 처리)
    review_list_test, original_review_list_test, product_list_test = \
        create_test_data(preprocessed_reviews, original_reviews, product_names, sample_size='max')
